...
This section defines the named types for each data asset, message request, and message response. It is an array of valid avro @record@ schemas, and adheres to the 1.4.1 avro specification. Each entry in the array is called a @type@. Its fields may be composed of primitive types (eg. string, int, float) as well as other named types so long as they are defined _previously_ in the @types@ array. Any referenced types must be defined in the types array before any definitions that refer to them. A @type@ definition can have the following fields:
...
* @request@     - An array of arguments to the message. A request has two fields, a @name@ and a @type@ where the @type@ is defined in the @types@ section. All infochimps API calls take a single argument of either a named @record@ type or a @map@ (hash).
* @response@    - The named @type@ (defined in the protocol's @types@ section) for the response.
* @errors@      - An optional array of named types that this message throws as errors.
* @doc@         - Documentation specific to this message
* @samples@     - Sample call request and response/error contents (see below).
...
* @request@     - an array of one element, that being a hash suitable for populating the message's @request@ type.
* @response@    - a hash suitable for populating the message's @response@ type.
* @url@         - Rather than supply the request params, you may supply them as a URL.
...
* @tags@        - An array of tags describing the catalog entry. Tags may only contain characters within [a-z0-9_] (lowercase alphanumeric or underscore). The publishing system MAY augment these tags with additional ones extracted from the protocol as a whole.
...
An array of HbaseTarget hashes. Each HabaseTarget hash describes how to map one or more data_assets into an Hbase database. It contains the following fields:
...
* @id_field@    - The name of a field withing @object_type@ to use as an inherent id during indexing (optional). If this field is omitted the records will be assigned an id dynamically during indexing.
